...
system("wget --read-timeout=20 -q ftp://ftp.ncbi.nlm.nih.gov/genomes/refseq/protozoa/assembly_summary.txt");
...
while(<IN>) {
	...
	if ($d[11] eq "Complete Genome" || $d[11] eq "Chromosome" || $d[11] eq "Scaffold") {
		my $ftppath = $d[19];
